...
The general idea behind PRP is to provide you with a templated workflow that is general enough so many of the individual steps required to complete the project  might be shared  with many analyses from various fields. 
...
This involved storage that you physically connect to your machine. Typically you have two options: harddrives permanently connected to your device and removable storage such as external hard drives and USB-drives. 
...
Data distributed in this manner is easy to share with collaborators. Typically, we are talking about small datasets.
...
As data grows or multiples users require access to the same data, it is common to host the data in a database, rather than a set of flat files. 
...
The data may exist on a different server physically located next to you or on the other side of the planet. We recommend a service called Globus to facilitate data sharing. Globus allows users to create _endpoints_ to that allows collaborators access to that data. 
...
#here the online source is the flat file stored on a Google Drive folder
#made accesible through through a public link
...
Once the data is ingested, it may require cleaning, reshaping, or creating new variables to complete a particular analysis. 
